...
public class Solution {
	...
	public static void main(String[] args) throws Exception {
		...
		str = br.readLine();
		TC = Integer.parseInt(str); // 테케 입력 50

		for (int tc = 1; tc <= TC; tc++) {
			str = br.readLine(); // 2 1 1406932606
			st = new StringTokenizer(str, " ");

			N = Integer.parseInt(st.nextToken()); // 간선 수
			Q = Integer.parseInt(st.nextToken()); // 자식수
			seed = Long.parseLong(st.nextToken());

			makeTree(N - 1); // 간선수(부자관계 수)전달 -> 인접행렬 생성
			usersolution.dfs_init(N, path);

			score = 100;
			for (int i = 1; i <= Q; i++) {
				str = br.readLine(); // 33 69
				st = new StringTokenizer(str, " ");

				K = Integer.parseInt(st.nextToken()); // 얘가 왕
				ans = Integer.parseInt(st.nextToken()); // 답
